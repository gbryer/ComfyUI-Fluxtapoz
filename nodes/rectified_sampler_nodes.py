...
def get_sample_reverse(latent_image, eta, start_time, end_time, eta_trend, mask=None):
    # Controlled Reverse ODE (Algorithm 2)
    # Store mask in closure
    stored_mask = mask.clone() if mask is not None else None
    ...
    def sample_reverse(model, y1, sigmas, extra_args=None, callback=None, disable=None):
        ...
        # If mask is provided, resize and expand it to match latent dimensions
        local_mask = None
        if stored_mask is not None:
            # Move mask to same device as X
            local_mask = stored_mask.to(X.device)
            
            # Resize mask to match latent dimensions
            import torch.nn.functional as F
            local_mask = F.interpolate(local_mask.unsqueeze(0), size=(X.shape[2], X.shape[3]), mode='bilinear', align_corners=False)
            local_mask = local_mask.squeeze(0)
            
            # Expand mask to match batch and channel dimensions
            local_mask = local_mask.expand(X.shape[0], -1, -1, -1)
        ...
        for i in trange(N, disable=disable):
            ...
            # 7. Controlled Vector field with mask modulation
            if local_mask is not None:
                # Apply mask to eta value
                current_eta = eta_values[i] * local_mask
            else:
                current_eta = eta_values[i]
            
            controlled_vector_field = unconditional_vector_field + current_eta * (conditional_vector_field - unconditional_vector_field)
...
class FluxReverseODESamplerNode:
    ...
    def build(self, model, latent_image, eta, start_step, end_step, eta_trend='constant', mask=None):
        process_latent_in = model.get_model_object("process_latent_in")
        latent_image = process_latent_in(latent_image['samples'])
        sampler = KSAMPLER(get_sample_reverse(latent_image, eta, start_step, end_step, eta_trend, mask))
